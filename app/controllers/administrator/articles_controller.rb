class Administrator::ArticlesController < Administrator::BaseController
    ...
    begin
      respond_to do |format|
        if @article.save && @article.images.each(&:save)
          format.html { redirect_to([:administrator, @article], :notice => t("notices.article_posted") ) }
    ...
    begin
      respond_to do |format|
        if @article.update_attributes(params[:article])
          format.html { redirect_to([:administrator, @article], :notice => t("notices.article_updated")) }
          format.xml  { head :ok }
        else
          format.html { render :action => "edit" }
          format.xml  { render :xml => @article.errors, :status => :unprocessable_entity }
  ...
  def destroy
    ...
    respond_to do |format|
      format.html { redirect_to(administrator_articles_url) }
      format.xml  { head :ok }
    end
  end
