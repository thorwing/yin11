class HomeController < ApplicationController
  ...
  def index
    ...
    @desired_tags = Tag.desc(:desires_count).limit(7)

    more_desires = Desire.recommended.tagged_with(@desired_tags.map(&:name)).desc(:priority)
    result = {}
    @desired_tags.each do |tag|
      result[tag.name] = more_desires.select{|d| d.tags.include?(tag.name) && d.get_image_url.present?}.take(10)
    end
    @desired_tags = @desired_tags.reject{|t| result[t.name].empty?}
    @desires = result.inject([]){|memo, (key, values)| memo | values}.compact.uniq
